...
function normalizeWasteCategory(value) {
...
function isListingAvailableForMarket(listing) {
  const status = String(listing.status || "available").toLowerCase();
  return status !== "locked" && status !== "escrowlocked" && status !== "paid";
}

function buildClustersFromListings(listings) {
  const validListings = listings.filter((listing) => {
    const wasteCategory = getWasteCategory(listing);
    return isListingAvailableForMarket(listing)
      && validNumber(listing.lat)
      && validNumber(listing.lng)
      && wasteCategory
      && listing.quantityTonnes !== undefined;
  });
  ...
  const byCrop = validListings.reduce((acc, listing) => {
    const wasteCategory = getWasteCategory(listing);
    const key = normalizeWasteCategory(wasteCategory);
    acc[key] ||= [];
    acc[key].push(listing);
    return acc;
  }, {});

  Object.entries(byCrop).forEach(([categoryKey, cropListings]) => {
    const categoryLabel = cropListings[0] ? getWasteCategory(cropListings[0]) : categoryKey;
    ...
    while (remaining.length) {
      ...
      for (let i = remaining.length - 1; i >= 0; i -= 1) {
        ...
        const sameProduct = normalizeWasteCategory(getWasteCategory(listing)) === categoryKey;
        if (sameProduct && haversineKm(seed, listing) <= MAX_CLUSTER_RADIUS_KM) {
      ...
      clusters.push({
        id: `cluster-${sanitizeCropId(categoryLabel)}-${clusterIndex}-${Math.round(clusterLat * 1000)}`,
        region: `${categoryLabel} cluster`,
        totalTonnes,
...
export async function getListingsByBuyer(buyerId) {
  if (!buyerId) {
    return [];
  }
...
export function subscribeListingsByBuyer(buyerId, callback) {
  if (!buyerId) {
    callback([]);
    return () => {};
  }
